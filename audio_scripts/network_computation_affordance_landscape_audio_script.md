...
This note was surfaced on August 3, 2026. The paper is by Suman S. Kulkarni, Jason Z. Kim, Panagiotis Fotiadis, Fabio Pasqualetti, and Dani S. Bassett, and it is an arXiv preprint.
...
An interaction matrix or weighted connectivity matrix for the network of interest, together with an operating point or state around which dynamics are linearized, the assumed control-input structure, and a target activity transition. In the applications, the inputs include a fly ring-attractor circuit, structural connectomes from one hundred Human Connectome Project subjects using Schaefer and Yeo network definitions, and recurrent weight matrices from artificial recurrent neural networks trained on decision-making tasks.
...
The preserved note is based on full-text inspection of the arXiv H T M L version. The applications use a modeled Drosophila head-direction ring attractor, structural connectivity from one hundred Human Connectome Project subjects with cortical subdivision into canonical networks and the seventeen-network Yeo atlas, and artificial recurrent neural networks with sixty-four recurrent units trained on five perceptual decision-making task variants.
...
Across one hundred Human Connectome Project subjects, landscape heterogeneity tracks the sensorimotor-association axis, with Spearman r sub s equal to minus zero point six four and p equal to zero point zero zero six across the seventeen-network Yeo subdivision.
Sensory networks show more heterogeneous landscapes, while association networks look more homogeneous.
Lower-cost modes tend to be more bilaterally symmetric, with median per-network Spearman correlations between mode cost and bilateral index ranging from minus zero point two three to minus zero point four six and Wilcoxon p values below ten to the minus sixteen.
...
The human result is anchored in one hundred subjects rather than in a tiny convenience sample.
